...
public class PhoneTypeDaoImpl extends HibernateDao implements PhoneTypeDao {
	...
	public List<PhoneType> list() {
		...
		List<PhoneType> phoneList = session.createQuery("from PhoneType")
				.list();
		session.close();
	...
	public PhoneType getPhoneTypeByDescription(String description) {
		Session session = getSessionFactory().openSession();
		Query query = session
				.createQuery("from PhoneType where phone_type_desc = :description");
	...
	public PhoneType getPhoneTypeById(Long id) {
		Session session = getSessionFactory().openSession();
		Query query = session
				.createQuery("from PhoneType where phone_type_id = :id");
