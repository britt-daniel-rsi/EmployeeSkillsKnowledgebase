...
public class UserDao {
    ...
    public Integer getMaxId() {
        ...
        SQLQuery query = session.createSQLQuery(
                "select max(contactId) from dbo.contacts");
    ...
    public List<User> SurnameSearch(String surname) {
        Session session = this.sessionFactory.openSession();
        List<User> personList = session.createQuery(
                "from User as user where user.surname='" + surname + "'").list();
        session.close();
    ...
    public List<User> IdSearch(Integer id) {
        Session session = this.sessionFactory.openSession();
        List<User> personList = session.createQuery(
                "from User as user where user.id='" + id + "'").list();
        session.close();
